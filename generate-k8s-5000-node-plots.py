...
def create_scenario_1_5000node_plots(results):
    ...
    fig.suptitle('Scenario 1: 5000-Node Byzantine Stress Test (50% Poisoned)', 
    ...
    ax1.axhline(y=80, color='red', linestyle='--', linewidth=2, alpha=0.7, label='80% Threshold')
    ...
    for r, acc in zip(rounds, accuracy):
        ax1.text(r, acc + 0.3, f'{acc:.2f}%', ha='center', fontsize=9, fontweight='bold')
    ...
    ax3 = axes[1, 0]
    ax3.hist(accuracy, bins=5, color='#2E86AB', alpha=0.7, edgecolor='black', linewidth=1.5)
    ax3.axvline(x=np.mean(accuracy), color='red', linestyle='--', linewidth=2.5, label=f'Mean: {np.mean(accuracy):.2f}%')
    ...
    ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes, fontsize=10,
            ...
            bbox=dict(boxstyle='round', facecolor='#E8F5E9', alpha=0.8, edgecolor='black', linewidth=1.5))
...
def create_scenario_2_scaling_plots(results):
    ...
    fig.suptitle('Scenario 2: Kubernetes Scaling Analysis (100-5000 Nodes)', 
                 fontsize=15, fontweight='bold')
    ...
    bars1 = ax1.bar(range(len(scales)), accuracies, color=colors, alpha=0.7, 
    ...
    ax2.plot(scales, efficiency, 'o-', linewidth=3, markersize=12, color='#2E86AB',
...
def create_scenario_3_threshold_plots(results):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle('Scenario 3: Byzantine Tolerance Threshold at 5000-Node Scale', 
                 fontsize=15, fontweight='bold')
    
    ratios = [float(t['byzantine_ratio']) * 100 for t in results['scenario_3']['threshold_tests']]
    ...
    ax1 = axes[0]
    colors = ['#06A77D' if v == 'PASS' else '#D62839' for v in verdicts]
    bars1 = ax1.bar(range(len(ratios)), accuracies, color=colors, alpha=0.7,
                    edgecolor='black', linewidth=1.5)
    ax1.axhline(y=80, color='red', linestyle='--', linewidth=2, alpha=0.7, label='80% Threshold')
    ...
    ax2.axhline(y=80, color='red', linestyle='--', linewidth=2, alpha=0.7, label='Failure Threshold')
    ax2.fill_between(ratios, accuracies, 80, where=[a >= 80 for a in accuracies],
                     alpha=0.3, color='#06A77D', label='Resilient Zone')
    ax2.fill_between(ratios, accuracies, 80, where=[a < 80 for a in accuracies],
                     alpha=0.3, color='#D62839', label='Failure Zone')
...
def create_scenario_4_intensity_plots(results):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle('Scenario 4: Attack Intensity Variation at 5000-Node Scale', 
                 fontsize=15, fontweight='bold')
    
    intensities = [int(t['attack_strength'] * 100) for t in results['scenario_4']['intensity_tests']]
    ...
    ax1 = axes[0]
    bars1 = ax1.bar(range(len(intensities)), accuracies, color='#2E86AB', alpha=0.7,
                    edgecolor='black', linewidth=1.5)
    ...
    ax1.set_title('Accuracy vs Attack Intensity (5000 nodes, 50% Byzantine)', fontweight='bold')
    ax1.grid(True, alpha=0.3, axis='y')
    ax1.set_ylim([85, 87])
    ...
    ax2 = axes[1]
    bars2 = ax2.bar(range(len(intensities)), degradations, color='#D62839', alpha=0.7,
                    edgecolor='black', linewidth=1.5)
    ...
    for bar, deg in zip(bars2, degradations):
        ...
        ax2.text(bar.get_x() + bar.get_width()/2., height + 0.1,
                f'{deg:.2f}%', ha='center', va='bottom', fontsize=10, fontweight='bold')
...
def create_combined_master_plot(results):
    ...
    fig.suptitle('5000-Node Byzantine Stress Test Suite - Complete Summary', 
    ...
    ax1.set_title('Scenario 1: 5000-Node 50% Byzantine (10 Rounds)', fontweight='bold', fontsize=13)
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim([84, 88])
    ...
    ax1b.text(0.1, 0.5, s1_stats, fontsize=11, fontweight='bold', verticalalignment='center', fontfamily='monospace',
             bbox=dict(boxstyle='round', facecolor=s1_color, alpha=0.3, edgecolor='black', linewidth=2))
    ...
    ax2.set_title('Scenario 2: Kubernetes Scaling (100-5000 Nodes)', fontweight='bold', fontsize=13)
    ax2.grid(True, alpha=0.3)
    ax2.set_ylim([84, 88])
    ax2.set_xscale('log')
    ...
    ax2b.axis('off')
    all_pass_s2 = all(t['verdict'] == 'PASS' for t in results['scenario_2']['scaling_tests'])
    ...
    ax2b.text(0.1, 0.5, s2_stats, fontsize=11, fontweight='bold', verticalalignment='center', fontfamily='monospace',
             bbox=dict(boxstyle='round', facecolor=s2_color, alpha=0.3, edgecolor='black', linewidth=2))
    
    # Scenario 3: Threshold
    ax3 = fig.add_subplot(gs[2, :2])
    ratios_s3 = [float(t['byzantine_ratio']) * 100 for t in results['scenario_3']['threshold_tests']]
    ...
    ax3.set_title('Scenario 3: Byzantine Tolerance Threshold (5000 Nodes)', fontweight='bold', fontsize=13)
    ...
    s3_passed = sum(1 for t in results['scenario_3']['threshold_tests'] if t['verdict'] == 'PASS')
    s3_color = '#06A77D'
    ...
    ax3b.text(0.1, 0.5, s3_stats, fontsize=11, fontweight='bold', verticalalignment='center', fontfamily='monospace',
             bbox=dict(boxstyle='round', facecolor=s3_color, alpha=0.3, edgecolor='black', linewidth=2))
