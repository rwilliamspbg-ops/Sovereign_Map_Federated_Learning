...
class RealisticNetworkSimulator:
    ...
    def print_stats(self):
        ...
        print(f"  Fast Messages: {self.fast_msgs} ({self.fast_msgs/self.total_msgs:.1%} < 10ms)")
        print(f"  Slow Messages: {self.slow_msgs} ({self.slow_msgs/self.total_msgs:.1%} >= 10ms)")
...
def check_convergence(accuracies, byzantine_pct):
    ...
    avg_last_10 = np.mean(accuracies[-10:]) if len(accuracies) >= 10 else np.mean(accuracies[-5:])
...
class TPMNodePoolWithMetrics:
    ...
    def print_crypto_stats(self):
        ...
        print(f"  Verified: {self.quotes_verified:,} ({self.quotes_verified/self.quotes_created:.1%})")
    ...
    def run_all(self):
        ...
        for bft in self.BFT_LEVELS:
            for attack in self.ATTACKS:
                ...
                print(f"[{status}] Acc: {result['final']:6.2f}% | Avg10: {result['avg_last_10']:6.2f}%")
