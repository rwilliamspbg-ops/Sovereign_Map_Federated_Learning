        ...
        while time.time() - start < max_wait:
            try:
                ...
                if num_tests > 0:
                    ...
                    print(f" | Latest: {last_test['byzantine_percentage']}% Byzantine "
                          f"({last_test['aggregation_method']}) | "
                          f"Final Acc: {last_test['final_accuracy']:.1f}% | "
                          f"Converged: {'✅' if last_test['converged'] else '❌'}", end="")
                ...
                # Check for stall
                if stalled_count > 10:
                    print(f"⚠️  No progress for {stalled_count * check_interval}s, checking backend...")
...
"""
        ...
        if summary:
            ...
            if critical:
                report += f"**Critical Threshold Found:** {critical}% Byzantine nodes\n\n"
                report += f"System convergence **FAILS** above this Byzantine percentage.\n\n"
        ...
        if results.get('tests'):
            for test in results['tests']:
                ...
                report += f"- **Converged:** {'✅ Yes' if test['converged'] else '❌ No'}\n"
                report += f"- **Stable:** {'✅ Yes' if test['stable'] else '⚠️  No'}\n"
                report += f"- **Max Accuracy Reached:** {test['max_accuracy']:.2f}%\n"
                report += f"- **Accuracy Variance:** {test['accuracy_variance']:.4f}\n\n"
