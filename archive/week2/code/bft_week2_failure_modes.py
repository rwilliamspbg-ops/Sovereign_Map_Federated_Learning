    ...
    def apply_failures(self, round_num):
        ...
        if self.mode == "random_dropout":
            ...
            if num_failed < self.N * 0.2:
                # Add new failures
                new_fails = int(base_fail * 1.5) if num_failed > base_fail else base_fail
...
class FailureModeTest:
    def __init__(self, num_nodes=75, failure_mode="random_dropout", failure_rate=1, bft_pct=0, rounds=20):
        self.N = num_nodes
        self.mode = failure_mode
        self.rate = failure_rate
...
if __name__ == "__main__":
    ...
    modes = ["random_dropout", "permanent_crash", "cascading", "byzantine_crash", "timeout"]
    scales = [75, 200]
    failure_rates = [1, 3, 5]
    bft_levels = [0, 20]
        ...
        for mode in modes:
            ...
            for fail_rate in failure_rates:
                for bft in bft_levels:
                    test = FailureModeTest(num_nodes=scale, failure_mode=mode, 
                                          failure_rate=fail_rate, bft_pct=bft, rounds=20)
            ...
            for res in mode_results:
                ...
                print(f"      {status} Fail {res['rate']}% + Byzantine {res['bft']}%: "
    ...
    print(f"  Convergence Rate: {conv_count}/{total_count} ({conv_count*100//total_count}%)")
    print(f"  Average Accuracy: {np.mean([r['final'] for r in results.values()]):.1f}%")
    print(f"  Average Active %: {np.mean([r['avg_active']/s for s, r in 
