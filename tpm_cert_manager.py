...
class TPMCertificateManager:
    ...
    def generate_node_cert(self, node_id: int, node_name: str) -> Tuple[str, str]:
        ...
        cert = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([
                ...
                x509.IPAddress(__import__('ipaddress').IPv4Address(f"172.25.0.{node_id % 254 + 2}")),
            ]),
            critical=False,
        ).add_extension(
            x509.KeyUsage(
    ...
    def get_trust_report(self) -> Dict:
        ...
        return {
            ...
            "verified_nodes": sum(1 for n in self.trust_store.values() if n.get("verified")),
...
class NodeAuthenticator:
    ...
    def verify_message(self, message: bytes, signature: bytes, peer_node_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to verify message signature from node {peer_node_id}: {e}")
            return False
    ...
    def verify_authenticated_message(self, message: Dict) -> bool:
        ...
        try:
            ...
            if expected_hash != message["message_hash"]:
                logger.warning(f"Message hash mismatch from node {message['from_node']}")
                return False
