...
from tpm_attestation import AttestationService, attestation_aware_aggregation, AttestationReportGenerator
...
class BFTTestWithAttestations:
    ...
    def run_attestation_round(
        ...
        aggregated, agg_stats = attestation_aware_aggregation(
            ...
            Byzantine_threshold=byzantine_pct / 100.0
        )
    ...
    def run_full_test(self):
        ...
        for byzantine_pct in self.BYZANTINE_PERCENTAGES:
            for agg_method in self.AGGREGATION_METHODS:
                ...
                logger.info(f"\n{'='*80}")
                logger.info(f"Configuration {config_num}/12: {byzantine_pct}% Byzantine + {agg_method}")
                logger.info(f"{'='*80}\n")
                ...
                stable = np.var(accuracies[-20:]) < 0.1 if len(accuracies) >= 20 else False
    ...
    def get_summary(self) -> Dict:
        ...
        for bft_pct in sorted(self.BYZANTINE_PERCENTAGES):
            configs_at_pct = [r for r in self.results if r['byzantine_percentage'] == bft_pct]
        ...
        summary = {
            ...
            'attestation_metrics': {
                ...
                'verification_success_rate': (
                    ...
                    if self.metrics['total_quotes'] > 0 else 0
                )
            },
            'by_byzantine_pct': {}
        }
        ...
        for bft_pct in self.BYZANTINE_PERCENTAGES:
            ...
            summary['by_byzantine_pct'][f'{bft_pct}%'] = {
                ...
                'avg_attestation_rate': np.mean([c['attestation_success_rate'] for c in configs]),
            }
