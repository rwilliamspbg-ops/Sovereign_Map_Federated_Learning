...
def generate_npu_comparison_plots(artifacts_dir: str, plots_dir: str):
    ...
    throughput = [
        cpu_data.get('throughput_rps', 0),
        npu_data.get('throughput_rps', 0)
    ]
    colors = ['#FF6B6B', '#4ECDC4']
    ax = axes[0, 0]
    bars = ax.bar(scenarios, throughput, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    if throughput[1] > 0 and throughput[0] > 0:
        ...
        ax.text(0.5, max(throughput) * 0.95, f'{speedup:.2f}x speedup', 
                ha='center', fontsize=11, fontweight='bold',
                bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
    
    for i, v in enumerate(throughput):
        ax.text(i, v + max(throughput) * 0.02, f'{v:.0f}', ha='center', fontweight='bold')

    # Plot 2: Latency Comparison
    latency = [
        cpu_data.get('avg_latency_ms', 0),
        npu_data.get('avg_latency_ms', 0)
    ]
    ax = axes[0, 1]
    bars = ax.bar(scenarios, latency, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    ...
    if latency[0] > 0 and latency[1] > 0:
        ...
        ax.text(0.5, max(latency) * 0.95, f'{improvement:.1f}% faster', 
                ha='center', fontsize=11, fontweight='bold',
                bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7))
    
    for i, v in enumerate(latency):
        ax.text(i, v + max(latency) * 0.02, f'{v:.2f}ms', ha='center', fontweight='bold')
    ...
    width = 0.35
    ax.bar(x - width/2, cpu_vals, width, label='CPU', alpha=0.7, color='#FF6B6B', edgecolor='black')
    ax.bar(x + width/2, npu_vals, width, label='NPU', alpha=0.7, color='#4ECDC4', edgecolor='black')
    ...
    summary_data = [
        ...
        ['CPU (%)', f"{cpu_data.get('cpu_percent', 0):.1f}", 
    ...
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/01-npu-performance-analysis.png", dpi=300, bbox_inches='tight')
...
def generate_throughput_plots(artifacts_dir: str, plots_dir: str):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('1000-Node Throughput & Latency Analysis', fontsize=16, fontweight='bold')

    latencies = [l * 1000 for l in throughput_data['latency']]  # Convert to ms

    # Plot 1: Latency Histogram
    ax = axes[0, 0]
    ax.hist(latencies, bins=50, color='#3498db', alpha=0.7, edgecolor='black', linewidth=1.5)
    ...
    ax.axvline(np.mean(latencies), color='red', linestyle='--', linewidth=2, label=f'Mean: {np.mean(latencies):.2f}ms')
    ax.axvline(np.median(latencies), color='green', linestyle='--', linewidth=2, label=f'Median: {np.median(latencies):.2f}ms')
    ax.legend()
    ax.grid(alpha=0.3)
    ...
    bars = ax.bar([f'p{p}' for p in percentiles], perc_values, color=colors_perc, alpha=0.7, edgecolor='black', linewidth=1.5)
...
"""
    ...
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/02-throughput-latency-analysis.png", dpi=300, bbox_inches='tight')
...
def generate_consensus_plots(artifacts_dir: str, plots_dir: str):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('1000-Node Consensus Efficiency Analysis', fontsize=16, fontweight='bold')
    ...
    ax.plot(rounds, msg_counts, marker='o', linewidth=2.5, markersize=8, color='#3498db')
    ...
    wedges, texts, autotexts = ax.pie([success_rate, failure_rate], labels=['Success', 'Failure'],
    ...
    ax.bar(rounds, round_durations, color='#9b59b6', alpha=0.7, edgecolor='black', linewidth=1.5)
    ax.set_xlabel('Consensus Round', fontweight='bold', fontsize=11)
...
"""
    ...
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/03-consensus-efficiency-analysis.png", dpi=300, bbox_inches='tight')
