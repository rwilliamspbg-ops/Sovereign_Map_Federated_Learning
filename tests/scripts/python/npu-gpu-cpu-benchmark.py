...
class DeviceTrainingBenchmark:
    ...
    def train_step(self, dataloader: DataLoader, epoch: int) -> float:
        ...
        for data, target in dataloader:
            try:
                ...
                data, target = data.to(torch.device("cpu")), target.to(
                    torch.device("cpu")
                )
    ...
    def benchmark(self, batch_size: int = 16, num_batches: int = 50) -> Dict:
        ...
        logger.info(
            f"  Batch size: {batch_size}, Batches: {num_batches}, Epochs: {self.num_epochs}"
        )
...
class DeviceComparison:
    ...
    def benchmark_all_devices(self) -> Dict:
        ...
        for device_type in devices_to_test:
            ...
            try:
                ...
                logger.info(
                    f"{device_type.upper()} completed: {device_result['avg_epoch_time_sec']:.3f}s/epoch"
                )
            except Exception as e:
    ...
    def generate_comparison_report(self, results: Dict) -> str:
        ...
        sorted_results = sorted(
            valid_results.items(), key=lambda x: x[1]["avg_epoch_time_sec"]
        )

        # Performance table
        report.append(
            f"{'Device':<15} {'Epoch Time (s)':<20} {'Throughput':<20} {'Speedup vs CPU':<20}"
        )
        report.append("-" * 75)
        ...
        for device_type, result in sorted_results:
            ...
            report.append(
                f"{device_type.upper():<15} {epoch_time:<20.3f} {throughput:<20.0f} {speedup:<20.2f}x"
            )
        ...
        for rank, (device_type, result) in enumerate(sorted_results, 1):
            ...
            report.append(
                f"{rank}. {device_type.upper():<15} {result['avg_epoch_time_sec']:.3f}s/epoch [{status}]"
            )
...
class NPUContentionTest:
    ...
    def run(self, num_batches: int = 20) -> Dict:
        """Run contention test"""
        logger.info(
            f"Starting {self.device.upper()} contention test with {self.num_nodes} nodes"
        )
        ...
        logger.info(
            f"Test complete: {successful} successful, {failed} failed in {total_time:.2f}s"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="NPU/GPU/CPU Acceleration Benchmark Suite"
    )
    parser.add_argument("--all", action="store_true", help="Run all benchmarks")
    parser.add_argument(
        "--compare-devices", action="store_true", help="Compare all available devices"
    )
    ...
    parser.add_argument(
        "--nodes", type=int, default=20, help="Number of nodes for contention test"
    )
