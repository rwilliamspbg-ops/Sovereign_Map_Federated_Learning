...
# Make canonical scripts executable
chmod +x tests/scripts/bash/test-incremental-scale.sh tests/scripts/bash/test-dashboard.sh tests/scripts/bash/commit-test-results.sh

# Review configuration
cat tests/config/test-config.env
```
...
Edit `tests/config/test-config.env` to customize:
