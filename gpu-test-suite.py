...
class GPUInfo:
    ...
    def get_devices() -> Dict[str, bool]:
        ...
        devices = {
            ...
            "npu": hasattr(torch, "npu") and torch.npu.is_available() if hasattr(torch, "npu") else False,
            "cpu": True,
        }
        return devices
...
class GPUContentionTest:
    ...
    def run(self, num_batches: int = 20) -> Dict:
        ...
        logger.info(f"Contention test complete: {successful} successful, {failed} failed")
...
class RoundLatencyBenchmark:
    ...
    def run(self, num_rounds: int = 10, batch_per_node: int = 10) -> Dict:
        ...
        logger.info(f"Round latency benchmark: {num_rounds} rounds, {self.num_nodes} nodes")
...
class GPUTestReport:
    ...
    def cpu_vs_gpu_benchmark(self) -> Dict:
        ...
        if torch.cuda.is_available():
            ...
            speedup = cpu_result["avg_epoch_time_sec"] / gpu_result["avg_epoch_time_sec"]
    ...
    def generate_report(self, benchmarks: Dict) -> str:
        ...
        if "cpu_vs_gpu" in benchmarks:
            ...
            report.append(f"  CPU avg epoch time: {cpu_gpu['cpu']['avg_epoch_time_sec']:.3f}s")
            if cpu_gpu["gpu"]:
                report.append(f"  GPU avg epoch time: {cpu_gpu['gpu']['avg_epoch_time_sec']:.3f}s")
        ...
        if "contention" in benchmarks:
            ...
            report.append(f"  Total throughput: {cont['total_throughput_samples_per_sec']:.0f} samples/sec")
            report.append("")
        ...
        if "round_latency" in benchmarks:
            ...
            report.append(f"  Min/Max: {rl['min_round_latency_sec']:.3f}s / {rl['max_round_latency_sec']:.3f}s")
...
def main():
    parser = argparse.ArgumentParser(description="GPU Acceleration Test Suite for Sovereign Map FL")
    parser.add_argument('--benchmark', action='store_true', help='Run CPU vs GPU benchmark')
    parser.add_argument('--contention', action='store_true', help='Run GPU contention test')
    parser.add_argument('--round-latency', action='store_true', help='Run round latency benchmark')
    parser.add_argument('--nodes', type=int, default=20, help='Number of nodes for contention/latency tests')
    parser.add_argument('--rounds', type=int, default=10, help='Number of FL rounds for latency test')
