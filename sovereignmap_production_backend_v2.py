...
fl_convergence_rate = Gauge(
    "sovereignmap_fl_convergence_rate", "Convergence rate (accuracy delta)"
)
...
def fl_round_endpoint():
    ...
    current_accuracy = min(
        99.5, base_accuracy + (fl_round_number * improvement_per_round) + noise
    )
    ...
    logger.info(
        f"FL round {fl_round_number} completed | Accuracy: {current_accuracy:.2f}% | Loss: {current_loss:.4f} | Convergence: {convergence_rate:+.2f}%",
        extra={
    ...
    return jsonify(
        {
            ...
            "current_accuracy": (
                convergence_history["accuracies"][-1]
                if convergence_history["accuracies"]
                else 0
            ),
            "current_loss": (
                convergence_history["losses"][-1]
                if convergence_history["losses"]
                else 0
            ),
        }
    )
...
if __name__ == "__main__":
    ...
    logger.info(
        "Sovereign Maps backend starting on port 5000 (with convergence tracking)"
    )
