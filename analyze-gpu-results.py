...
print("-" * 80)
print(
    f"{'Nodes':<10} {'Total Time (s)':<20} {'Avg Node Time (s)':<20} {'Throughput (samples/sec)':<25}"
)
print("-" * 80)
...
for nodes in [5, 10, 20, 30]:
    ...
    if key in results:
        data = results[key]
        print(
            f"{nodes:<10} {data['total_time_sec']:<20.2f} {data['avg_node_time_sec']:<20.3f} {data['total_throughput_samples_per_sec']:<25.0f}"
        )

print("\n2. ROUND LATENCY TEST RESULTS (Sequential FL Rounds)")
print("-" * 80)
print(
    f"{'Nodes':<10} {'Avg Latency (s)':<20} {'Min (s)':<15} {'Max (s)':<15} {'Updates/sec':<15}"
)
print("-" * 80)
...
for nodes in [5, 10, 20]:
    ...
    if key in results:
        data = results[key]
        print(
            f"{nodes:<10} {data['avg_round_latency_sec']:<20.3f} {data['min_round_latency_sec']:<15.3f} {data['max_round_latency_sec']:<15.3f} {data['updates_per_sec']:<15.2f}"
        )
...
print(
    f"{'Nodes':<10} {'Per-Node Time (s)':<20} {'Efficiency %':<20} {'Scaling Factor':<15}"
)
print("-" * 80)
...
if "benchmark-baseline" in results:
    print(
        f"CPU Baseline (single epoch): {results['benchmark-baseline']['cpu']['avg_epoch_time_sec']:.3f} seconds"
    )
    print(
        f"CPU Throughput: {results['benchmark-baseline']['cpu']['samples_per_sec']:.0f} samples/sec"
    )
    print()

print(f"Contention Test Results:")
print(
    f"  - 5 nodes:  {contention_results[5]['total_throughput_samples_per_sec']:.0f} total samples/sec"
)
print(
    f"  - 10 nodes: {contention_results[10]['total_throughput_samples_per_sec']:.0f} total samples/sec"
)
print(
    f"  - 20 nodes: {contention_results[20]['total_throughput_samples_per_sec']:.0f} total samples/sec"
)
print(
    f"  - 30 nodes: {contention_results[30]['total_throughput_samples_per_sec']:.0f} total samples/sec"
)
...
print(
    f"{'Nodes':<10} {'Latency (s)':<15} {'Latency/Node (ms)':<20} {'Updates/sec':<15}"
)
print("-" * 80)
for nodes in sorted(round_results.keys()):
    ...
    print(
        f"{nodes:<10} {data['avg_round_latency_sec']:<15.3f} {latency_per_node:<20.1f} {total_throughput:<15.2f}"
    )
...
for nodes in [5, 10, 20, 30]:
    ...
    if key in results:
        ...
        print(
            f"  {nodes:2d} nodes: {actual:7.0f} samples/sec (linear: {linear_expected:7.0f}, efficiency: {efficiency_percent:5.1f}%)"
        )
...
for nodes in sorted(round_results.keys()):
    data = round_results[nodes]
    print(
        f"  {nodes:2d} nodes: {data['avg_round_latency_sec']:.3f}s per round ({data['updates_per_sec']:.1f} updates/sec)"
    )
