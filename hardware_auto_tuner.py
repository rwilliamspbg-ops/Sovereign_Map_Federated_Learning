...
class FLNodeConfig:
    ...
    def scale_down(self):
        ...
        logger.info(f"Scaled DOWN: Batch Size={self.batch_size}, Threads={self.cuda_threads}")

    def scale_up(self):
        self.batch_size = min(256, self.batch_size * 2)
        self.cuda_threads = min(4096, self.cuda_threads * 2)
        logger.info(f"Scaled UP: Batch Size={self.batch_size}, Threads={self.cuda_threads}")
...
def get_current_metric(metric_name):
    ...
    try:
        ...
        import random
        return random.uniform(10, 95) if "utilization" in metric_name else random.uniform(1, 30)
...
def tuning_loop():
    ...
    while True:
        ...
        logger.info(f"Metrics: GPU_Util={gpu_util_pct:.1f}%, Privacy_Overhead={privacy_overhead_pct:.1f}%")
        ...
        if gpu_util_pct and gpu_util_pct > 90.0:
            ...
            logger.warning("High DP overhead detected. Scaling down threads to prevent latency block.")
            config.scale_down()
