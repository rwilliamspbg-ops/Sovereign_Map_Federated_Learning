...
func Prometheus(r Result, scenario string) string {
	...
	fmt.Fprintf(&b, "sovereign_simulator_nodes{%s} %d\n", labels, r.NodeCount)
	...
	fmt.Fprintf(&b, "sovereign_simulator_rounds_requested_total{%s} %d\n", labels, r.RoundsRequested)
	...
	fmt.Fprintf(&b, "sovereign_simulator_rounds_completed_total{%s} %d\n", labels, r.RoundsCompleted)
	...
	fmt.Fprintf(&b, "sovereign_simulator_straggler_events_total{%s} %d\n", labels, r.StragglerEvents)
	...
	fmt.Fprintf(&b, "sovereign_simulator_malicious_events_total{%s} %d\n", labels, r.MaliciousNodeEvents)
	...
	fmt.Fprintf(
		&b,
		"sovereign_simulator_average_round_duration_seconds{%s} %.6f\n",
		labels,
		r.AverageRoundDuration.Seconds(),
	)
