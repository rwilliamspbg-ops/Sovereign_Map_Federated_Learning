...
def create_comprehensive_plot(results, metrics):
    ...
    fig.suptitle('Byzantine Stress Test - Comprehensive Performance Analysis', 
                 fontsize=16, fontweight='bold')
    
    # 1. Model Accuracy Comparison
    ax1 = axes[0, 0]
    ax1.plot(metrics['rounds'], metrics['global_accuracy'], 'o-', linewidth=2.5, 
             markersize=8, label='Global Model', color='#2E86AB')
    ax1.plot(metrics['rounds'], metrics['honest_accuracy'], 's--', linewidth=2, 
             markersize=7, label='Honest Nodes Avg', color='#06A77D')
    ax1.plot(metrics['rounds'], metrics['malicious_accuracy'], '^:', linewidth=2, 
             markersize=7, label='Malicious Nodes Avg', color='#D62839')
    ax1.axhline(y=80.0, color='red', linestyle='--', linewidth=1.5, alpha=0.7, label='Resilience Threshold (80%)')
    ...
    ax2 = axes[0, 1]
    colors = ['#06A77D' if rate >= 90 else '#F77F00' for rate in metrics['detection_rate']]
    bars2 = ax2.bar(metrics['rounds'], metrics['detection_rate'], color=colors, alpha=0.7, edgecolor='black', linewidth=1.5)
    ax2.axhline(y=90.0, color='red', linestyle='--', linewidth=2, alpha=0.7, label='Detection Threshold (90%)')
    ...
    bars3 = ax3.bar(metrics['rounds'], metrics['latency'], color=colors3, alpha=0.7, edgecolor='black', linewidth=1.5)
    ax3.axhline(y=10.0, color='red', linestyle='--', linewidth=2, alpha=0.7, label='Latency Threshold (10ms)')
    ...
    table_data = [
        ...
        ['Accuracy Min/Max', f"{summary['accuracy']['min']:.2f}% / {summary['accuracy']['max']:.2f}%", 'PASS'],
        ['', '', ''],
        ['Byzantine Detection', f"{summary['byzantine_detection']['avg']:.1f}% (Avg)", 'PASS'],
        ['', '', ''],
        ['Latency Avg/Max', f"{summary['performance']['latency_ms_avg']:.2f}ms / {summary['performance']['latency_ms_max']:.2f}ms", 'PASS'],
        ['', '', ''],
        ['Success Rate', f"{summary['success_rate']:.1f}% ({summary['passed_rounds']}/{summary['total_rounds']} rounds)", 'PASS'],
        ['', '', ''],
        ['Overall Verdict', 'RESILIENT', 'PASS'],
    ]
    
    table = ax4.table(cellText=table_data, cellLoc='left', loc='center',
                     colWidths=[0.35, 0.35, 0.20])
    ...
    ax4.set_title('Summary Statistics & Verdicts', fontweight='bold', fontsize=12, pad=20)
...
def create_attack_analysis_plot(results):
    ...
    fig.suptitle('Byzantine Attack Analysis - 50% Poisoned Gradients', 
    ...
    ax2 = axes[1]
    bars = ax2.bar(rounds, total_attacks, color='#D62839', alpha=0.7, edgecolor='black', linewidth=1.5)
    ax2.set_xlabel('Round Number', fontweight='bold')
...
def create_resilience_summary_plot(results):
    ...
    bars = ax.barh(y_pos, [1]*len(verdict_names), color=colors, alpha=0.7, edgecolor='black', linewidth=2)
    
    # Add checkmarks and status text
    for i, (bar, verdict) in enumerate(zip(bars, verdicts)):
        symbol = '[OK]' if verdict == 'PASS' else '[FAIL]'
        ax.text(0.5, i, f'{symbol} {verdict}', ha='center', va='center', 
    ...
    ax.set_title(f'Byzantine Resilience Test Verdicts\n({pass_count}/{len(verdicts)} Criteria Met)', 
