...
func (vm *SmartContractVM) executeGovernanceFunction(contract *Contract, ctx *ExecutionContext, functionName string) (map[string]interface{}, error) {
	...
	case "createProposal":
		...
		action := ""
		if raw, ok := ctx.CallData["action"]; ok {
			action, _ = raw.(string)
		}
		params := map[string]interface{}{}
		if raw, ok := ctx.CallData["params"]; ok {
			if cast, ok := raw.(map[string]interface{}); ok {
				params = cast
			}
		}
		minVotes := uint64(1)
		if raw, ok := ctx.CallData["min_votes"]; ok {
			switch v := raw.(type) {
			case uint64:
				minVotes = v
			case int:
	...
	case "executeProposal":
		proposalID := ctx.CallData["proposal_id"].(string)
		key := fmt.Sprintf("proposal:%s", proposalID)
		val, ok := contract.State.Get(key)
		if !ok {
			return nil, fmt.Errorf("proposal not found: %s", proposalID)
		}

		proposal := val.(map[string]interface{})
		if status, ok := proposal["status"].(string); ok && status != "active" {
			return nil, fmt.Errorf("proposal not active: %s", status)
		}

		forVotes, _ := proposal["for_votes"].(uint64)
		againstVotes, _ := proposal["against_votes"].(uint64)
		minVotes := uint64(1)
		if mv, ok := proposal["min_votes"].(uint64); ok {
			minVotes = mv
		}

		if forVotes < minVotes || forVotes <= againstVotes {
			proposal["status"] = "rejected"
			contract.State.Set(key, proposal)
			return map[string]interface{}{"success": false, "reason": "insufficient support"}, nil
		}
		...
		action := ""
		if raw, ok := proposal["action"].(string); ok {
			action = raw
		}
		params := map[string]interface{}{}
		if raw, ok := proposal["params"].(map[string]interface{}); ok {
			params = raw
		}
		...
		ctx.Events = append(ctx.Events, ContractEvent{
			...
			Parameters: map[string]interface{}{
				"proposal_id": proposalID,
				"action":      action,
				"executor":    ctx.CallerAddress,
			},
			Timestamp: time.Now().Unix(),
		})

		result := map[string]interface{}{"success": true, "action": action, "proposal_id": proposalID}
		if len(params) > 0 {
