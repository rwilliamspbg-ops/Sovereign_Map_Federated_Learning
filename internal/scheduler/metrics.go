...
func (m *Metrics) Prometheus(nodeID string) string {
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_rounds_started_total{%s} %d\n", labels, m.roundsStarted))
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_rounds_completed_total{%s} %d\n", labels, m.roundsCompleted))
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_timeouts_total{%s} %d\n", labels, m.timeouts))
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_stragglers_total{%s} %d\n", labels, m.stragglers))
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_last_round_id{%s} %d\n", labels, m.lastRoundID))
	...
	b.WriteString(fmt.Sprintf("sovereign_scheduler_last_update_unix{%s} %d\n", labels, m.lastUpdated.Unix()))
	return b.String()
}
