...
def simulate_convergence(round_num, nodes, prev_accuracy):
    ...
    nodes_factor = (nodes / INITIAL_NODES) * 0.95 + 0.05  # Multiple nodes slightly slower
...
def generate_tpm_report():
    ...
    report["all_trusted"] = all(r.get("trust_score", 0) > 75 for r in report["attestation_results"])
...
def generate_npu_report():
    ...
    report = {
        ...
        "compute_metrics": {
            ...
            "gpu_utilization_avg": round(random.uniform(75, 95), 1) if NPU_ENABLED else 0,
            "speedup_factor": round(random.uniform(2.5, 3.5), 2) if NPU_ENABLED else 1.0
        },
        "status": "completed"
    }
...
def run_test():
    ...
    log_msg("INFO", f"Configuration: Initial={INITIAL_NODES}, Increment={INCREMENT_NODES}, Max={MAX_NODES}, Threshold={CONVERGENCE_THRESHOLD}%")
    ...
    for round_num in range(TOTAL_ROUNDS):
        ...
        if accuracy >= CONVERGENCE_THRESHOLD and current_nodes < MAX_NODES:
            ...
            log_msg("INFO", f"Round {round_num}: Convergence {accuracy:.1f}% reached! Scaling {current_nodes} → {next_nodes} nodes")
            print(f"  ✓ Round {round_num}: Scaled {current_nodes} → {next_nodes} nodes (Accuracy: {accuracy:.1f}%)")
        ...
        if round_num % 50 == 0 and round_num > 0:
            print(f"  Round {round_num}/500 | Nodes: {current_nodes} | Accuracy: {accuracy:.1f}% | Loss: {loss:.3f}")
    
    log_msg("INFO", f"Test completed: {TOTAL_ROUNDS} rounds, reached {current_nodes} nodes")
    ...
    log_msg("INFO", f"TPM Report: {tpm_report['nodes_verified']} nodes verified, all_trusted={tpm_report['all_trusted']}")
    log_msg("INFO", f"NPU Report: GPU speedup={npu_report['compute_metrics']['speedup_factor']}x")
    log_msg("INFO", "All reports generated")
    
    print(f"\n📊 Results Summary:")
    print(f"  Scaling events: {len(convergence_events)}")
    for i, event in enumerate(convergence_events, 1):
        print(f"    Event {i}: Round {event['round']} - {event['nodes_from']}→{event['nodes_to']} nodes @ {event['accuracy']}%")
