...
class MohawkNode:
    ...
    def attack_gradient(self, honest_gradient):
        ...
        poisoned_gradient = (
            1.0 - self.attack_strength
        ) * honest_gradient + self.attack_strength * inverted
...
class ByzantineStressTestSuite:
    ...
    def run_scenario_1_1000node_50percent(self):
        ...
        for i in range(total_nodes):
            ...
            node = MohawkNode(
                node_id=f"s1_node_{i}", malicious=is_malicious, attack_strength=1.0
            )
            nodes.append(node)
        ...
        for round_num in range(1, rounds + 1):
            ...
            detected_byzantine = (
                len(collected_updates) - 2 * aggregator.trim_count
                if aggregator.trim_count
                else 0
            )
            detection_rate = (
                (detected_byzantine / num_malicious * 100) if num_malicious > 0 else 0
            )
            ...
            print(
                f"   Global Accuracy: {global_accuracy:.2f}% | Detection: {detection_rate:.1f}% | Latency: {aggregation_time:.2f}ms"
            )
    ...
    def run_scenario_2_tolerance_threshold(self):
        ...
        print(
            f"SCENARIO 2: BYZANTINE TOLERANCE THRESHOLD TEST ({min_ratio}%-{max_ratio}%)"
        )
        print("=" * 90)
        print("Finding system breaking point by varying Byzantine node ratios")
        print(
            "Testing:",
            ", ".join([f"{int(r*100)}%" for r in self.threshold_ratios]),
            "Byzantine nodes",
        )
        print("=" * 90)
        ...
        for ratio in ratios:
            ...
            for round_num in range(1, rounds_per_ratio + 1):
                ...
                for node in nodes:
                    honest_gradient = node.train_local()
                    gradient = (
                        node.attack_gradient(honest_gradient)
                        if node.malicious
                        else honest_gradient
                    )
                    collected_updates.append(
                        {
                            "id": node.node_id,
            ...
            print(
                f"   Avg Accuracy: {avg_accuracy:.2f}% | Success: {success_rate:.0f}% ({passed}/{rounds_per_ratio})"
            )
        ...
        results["breaking_point_pct"] = (
            f"{breaking_point*100:.0f}%" if breaking_point else "Not found in range"
        )
        ...
        for test in results["threshold_tests"]:
            ...
            print(
                f"   {test['byzantine_ratio_pct']:>5} | {bar} | {test['accuracy_avg']:6.2f}% | {status}"
            )
    ...
    def run_scenario_3_attack_intensity_variation(self):
        ...
        for strength in attack_strengths:
            ...
            print(
                f"   Avg Accuracy: {avg_accuracy:.2f}% | Degradation: {test_result['degradation_from_clean']:.2f}%"
            )
        ...
        for test in results["intensity_tests"]:
            ...
            bar = "#" * bar_length + "-" * (20 - bar_length)
            print(
                f"   {test['attack_strength_pct']:>5} | {bar} | {test['accuracy_avg']:6.2f}% | -{test['degradation_from_clean']:5.2f}%"
            )
    ...
    def _generate_master_summary(self, s1_time, s2_time, s3_time, total_time):
        ...
        summary = {
            ...
            "scenario_2": {
                ...
                "tests_passed": sum(
                    1
                    for t in self.results["scenario_2"]["threshold_tests"]
                    if t["verdict"] == "PASS"
                ),
        ...
        print(f"\n[SCENARIO 1] 1000-node 50% Byzantine")
        print(
            f"   Time: {s1_time:.1f}s | Verdict: {summary['scenario_1']['verdict']} | Accuracy: {summary['scenario_1']['accuracy']}"
        )
        print(f"   Success Rate: {summary['scenario_1']['success_rate']}")
        print(f"\n[SCENARIO 2] Byzantine Tolerance Threshold")
        print(
            f"   Time: {s2_time:.1f}s | Breaking Point: {summary['scenario_2']['breaking_point']}"
        )
        print(
            f"   Tests Passed: {summary['scenario_2']['tests_passed']}/{summary['scenario_2']['tests_total']}"
        )
        print(f"\n[SCENARIO 3] Attack Intensity Variation")
        print(
            f"   Time: {s3_time:.1f}s | Levels Tested: {summary['scenario_3']['intensity_levels']}"
        )
...
def main():
    ...
    try:
        threshold_ratios = [
            ...
            for value in args.threshold_ratios.split(",")
            if value.strip()
        ]
    except ValueError as err:
        raise ValueError(
            "Invalid --threshold-ratios. Use comma-separated numeric percents, e.g. 70,80,90"
        ) from err
    ...
    for ratio in threshold_ratios:
        if ratio <= 0.0 or ratio >= 1.0:
            raise ValueError(
                "Each threshold ratio must be between 0 and 100 (exclusive)"
            )
