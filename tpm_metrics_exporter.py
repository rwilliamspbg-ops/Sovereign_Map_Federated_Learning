...
class TPMMetricsExporter:
        ...
        try:
            ...
            for node_name, node_info in report['nodes'].items():
                try:
                    ...
                    # Parse expiry time
                    expires_at = datetime.fromisoformat(node_info['expires_at'].replace('Z', '+00:00'))
                    seconds_until_expiry = (expires_at - datetime.utcnow()).total_seconds()
                    self.cert_expiry_seconds.labels(node_id=node_id).set(max(0, seconds_until_expiry))
                    
                    # Parse creation time
                    created_at = datetime.fromisoformat(node_info['created_at'].replace('Z', '+00:00'))
                    ...
                    is_revoked = 1 if node_info['serial'] in self.cert_manager.crl else 0
    ...
    def record_message_verified(self, from_node_id: int, to_node_id: int = None):
        ...
        self.messages_verified_total.labels(node_id=to_node_id, from_node_id=from_node_id).inc()
