...
def create_scenario_1_plots(results):
    ...
    malicious_acc = [
        r["accuracy"]["malicious_avg"] for r in results["scenario_1"]["rounds"]
    ]
    latency = [
        r["performance"]["aggregation_latency_ms"]
        for r in results["scenario_1"]["rounds"]
    ]
    detection = [
        r["detection"]["detection_rate"] for r in results["scenario_1"]["rounds"]
    ]
    ...
    ax2 = axes[0, 1]
    bars = ax2.bar(
        rounds, latency, color="#F77F00", alpha=0.7, edgecolor="black", linewidth=1.5
    )
    ax2.axhline(
        y=10,
        color="red",
...
def create_scenario_2_plots(results):
    ...
    min_ratio = (
        min(
            float(t["byzantine_ratio"])
            for t in results["scenario_2"]["threshold_tests"]
        )
        * 100
    )
    max_ratio = (
    ...
    fig.suptitle(
        f"Scenario 2: Byzantine Tolerance Threshold Test ({min_ratio:.0f}%-{max_ratio:.0f}%)",
        fontsize=14,
        fontweight="bold",
    )

    ratios = [
        t["byzantine_ratio_pct"] for t in results["scenario_2"]["threshold_tests"]
    ]
    accuracies = [t["accuracy_avg"] for t in results["scenario_2"]["threshold_tests"]]
    success_rates = [
        t["success_rate"] for t in results["scenario_2"]["threshold_tests"]
    ]
...
def create_scenario_3_plots(results):
    ...
    intensities = [
        t["attack_strength_pct"] for t in results["scenario_3"]["intensity_tests"]
    ]
    accuracies = [t["accuracy_avg"] for t in results["scenario_3"]["intensity_tests"]]
    degradations = [
        t["degradation_from_clean"] for t in results["scenario_3"]["intensity_tests"]
    ]
...
def create_combined_summary_plot(results):
    ...
    fig.suptitle(
        "Byzantine Stress Test Suite - Complete Summary", fontsize=16, fontweight="bold"
    )
    ...
    s2_passed = sum(
        1 for t in results["scenario_2"]["threshold_tests"] if t["verdict"] == "PASS"
    )
    ...
    intensities_s3 = [
        ...
        for t in results["scenario_3"]["intensity_tests"]
    ]
