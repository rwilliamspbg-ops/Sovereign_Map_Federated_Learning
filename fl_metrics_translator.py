...
class FLMetricsTranslator:
    ...
    def get_aggregated_stats(self) -> Dict:
        ...
        return {
            ...
            "node_status": {
                "healthy": sum(1 for m in self.metrics_cache.values() 
                             if m.convergence >= 70),
                "degraded": sum(1 for m in self.metrics_cache.values() 
                               if 50 <= m.convergence < 70),
                "critical": sum(1 for m in self.metrics_cache.values() 
                               if m.convergence < 50),
