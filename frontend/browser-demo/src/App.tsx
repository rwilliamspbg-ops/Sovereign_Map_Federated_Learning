...
const App: React.FC = () => {
  const [nodes, setNodes] = useState<{ id: string; active: boolean; gpu: boolean }[]>([]);
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', margin: '40px' }}>
      ...
      <p>Interactive Browser-based Federated Learning with WebGPU Acceleration.</p>
      
      <button 
        onClick={addNode} 
        style={{ padding: '10px 20px', fontSize: '16px', background: '#0070f3', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}
      >
        + Add Edge Node
      </button>
      ...
      {nodes.length > 0 && (
        <div style={{ marginTop: '40px', padding: '20px', background: '#f5f5f5', borderRadius: '8px' }}>
          ...
          <p>Total Gradients Compressed: {(nodes.length * 4.2).toFixed(1)} MB</p>
          <p>Bandwidth Saved: {(nodes.length * 4.2 * 0.9).toFixed(1)} MB (90% Reduction)</p>
        </div>
      )}
    </div>
  );
