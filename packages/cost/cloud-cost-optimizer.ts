...
export class CloudCostTracker {
    ...
    public generateROISummary(): CostReport {
        ...
        return {
            ...
            dollarsSavedAWS: parseFloat((savedGB * AWS_EGRESS_COST_PER_GB).toFixed(2)),
            dollarsSavedGCP: parseFloat((savedGB * GCP_EGRESS_COST_PER_GB).toFixed(2)),
        };
    }

    public reportToOpsLogger(): void {
        const report = this.generateROISummary();
        console.log(`[CostOptimizer] Bandwidth Saved: ${report.bandwidthSavedGB} GB`);
        console.log(`[CostOptimizer] Estimated Egress Savings: $${report.dollarsSavedAWS} (AWS) / $${report.dollarsSavedGCP} (GCP)`);
    }
}
