...
const mockAggregatorOn = vi.fn(
  (_event: string, _handler: (updates: unknown[]) => Promise<void>) => undefined
);
...
vi.mock("@sovereignmap/privacy", () => {
  class PrivacyEngine {
    ...
    public apply = vi.fn(async (u: Record<string, unknown>) => ({
      ...u,
      noiseApplied: true,
    }));
    public on = vi.fn(
      (event: string, cb: (remaining: number, total: number) => void) => {
...
vi.mock("@sovereignmap/island", () => {
  class IslandModeManager {
    public initialize = vi.fn(async () => undefined);
    public queueUpdate = vi.fn(async () => ({
      updateId: "q1",
      status: "queued",
      proof: "p",
    }));
    ...
    public syncWithState = vi.fn(
      async (_state: Record<string, unknown>) => undefined
    );
    public getStatus = vi.fn(() => ({
      mode: "online",
      updatesQueued: 0,
...
vi.mock("@sovereignmap/consensus", () => {
  class ConsensusParticipant {
    public initialize = vi.fn(async () => undefined);
    public submitUpdate = vi.fn(
      async (_round: string, _update: Record<string, unknown>) => ({
        accepted: true,
        reward: 1,
        roundId: "r1",
      })
...
describe("SovereignNode lifecycle branches", () => {
  ...
  it("handles network callbacks for disconnect/reconnect/byzantine branches", async () => {
    ...
    expect(node.metrics.recordByzantineFault).toHaveBeenCalledWith(
      "bad-node",
      "equivocation"
    );
  });
  ...
  it("covers aggregator branches and shutdown handler registration", async () => {
    ...
    await expect(nodeNone.becomeAggregator({ tier: "edge" })).rejects.toThrow(
      "Node not configured as aggregator"
    );
  ...
  it("rethrows submit errors from underlying privacy/network flow", async () => {
    ...
    await expect(
      node.submitMapUpdate({ location: { lat: 1, lng: 2 }, quality: 0.4 })
  ...
  it("covers aggregator init failure, shutdown failure path, and metrics collector branches", async () => {
    ...
    mockAggregatorStart.mockRejectedValueOnce(
      new Error("aggregator start failed")
    );
    await expect(node.becomeAggregator({ tier: "edge" })).rejects.toThrow(
      "aggregator start failed"
    );
  ...
  it("covers aggregate callback path and submitAggregate broadcast", async () => {
    ...
    const aggregateHandler = mockAggregatorOn.mock.calls.find(
      (c) => c[0] === "aggregate"
    )?.[1];
    if (!aggregateHandler) {
  ...
  it("covers wasm proof fallback and signal handler exit branches", async () => {
    ...
    const sigterm = onSpy.mock.calls.find((c) => c[0] === "SIGTERM")?.[1] as
      | (() => Promise<void>)
      | undefined;
