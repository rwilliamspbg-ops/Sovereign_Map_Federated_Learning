...
describe("PrivacyEngine", () => {
  ...
  it("detects GPU acceleration", async () => {
    ...
    expect(["cuda", "rocm", "ascend", "webgpu", "simd", "cpu"]).toContain(
      stats?.device
    );
  ...
  it("generates valid Gaussian noise with correct statistics", async () => {
    ...
    const mean =
      Array.from(samples).reduce((a, b) => a + b, 0) / samples.length;
    const variance =
      Array.from(samples).reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
      samples.length;
