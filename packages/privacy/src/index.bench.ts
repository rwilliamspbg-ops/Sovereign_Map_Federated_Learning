...
function benchmark(
  ...
  console.log(`\n${name}:`);
  console.log(
    `  Mean: ${mean.toFixed(3)}ms, Min: ${min.toFixed(3)}ms, Max: ${max.toFixed(
      3
    )}ms, StdDev: ${stdDev.toFixed(3)}ms`
  );
...
describe("GPU Acceleration Performance Benchmarks", async () => {
  it("compares Gaussian noise generation performance across dimensions", async () => {
    ...
    for (const dim of dimensions) {
      const cpuResult = benchmark(
        `CPU Gaussian (${dim} dim)`,
        () => {
      ...
      const gpuResult = benchmark(
        `GPU Gaussian (${dim} dim)`,
        () => {
  ...
  it("measures privacy engine end-to-end performance with GPU", async () => {
    ...
    const result = benchmark(
      "Full privacy apply (10K point cloud)",
      () => {
        engine
          .apply({
    ...
    console.log(
      `   Cost for 1000 nodes: ${(cost1000Nodes / 1000).toFixed(1)}ms per round`
    );
    console.log(
      `   Cost for 5000 nodes: ${(cost5000Nodes / 1000).toFixed(1)}ms per round`
    );
  ...
  it("demonstrates privacy overhead reduction with GPU", async () => {
    ...
    console.log(
      `   Observed CPU overhead: ~2400% (np.random.normal dominated)`
    );
    ...
    console.log(
      `   Gap to matrix multiply (0.1ms): ${(elapsed / 0.1).toFixed(0)}x`
    );
