...
describe("CompressionIntegration", () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 100; i++) {
      sampleGradient[i] =
        Math.random() * 0.01 + (Math.random() < 0.05 ? 0.5 : 0);
    }
  });
  ...
  describe("Compression", () => {
    ...
    it("should achieve target compression ratio", () => {
      ...
      expect(result.data.byteLength).toBeLessThan(result.stats.originalSize);
    });
  ...
  describe("Statistics", () => {
    ...
    it("should track total bytes", () => {
      ...
      const stats = integration.getStatistics();
      expect(stats.totalOriginalBytes).toBe(sampleGradient.byteLength * 2);
  ...
  describe("Integration", () => {
    ...
    it("should handle multiple sequential compressions", () => {
      ...
      const results = gradients.map((g) => integration.compressGradient(g));
