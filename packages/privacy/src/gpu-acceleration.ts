 ...
 */
export type AcceleratorType = 'cuda' | 'rocm' | 'ascend' | 'webgpu' | 'simd' | 'cpu';
...
export class AcceleratorDetector {
  ...
  private static checkCUDA(): boolean {
    try {
      ...
      return cuda && typeof cuda.isAvailable === 'function' && cuda.isAvailable();
    } catch {
      return false;
    }
  }
  ...
  private static checkROCm(): boolean {
    try {
      ...
      return rocm && typeof rocm.isAvailable === 'function' && rocm.isAvailable();
    } catch {
      return false;
    }
  }
...
export class GPUNoiseGenerator {
   ...
   */
  private generateGaussianAscend(dimension: number, sigma: number): Float64Array {
    try {
   ...
   */
  private generateGaussianWebGPU(dimension: number, sigma: number): Float64Array {
   ...
   */
  private generateLaplaceAscend(dimension: number, scale: number): Float64Array {
    try {
      const mindspore = require('mindspore');
      const ops = mindspore.ops;
   ...
   */
  private generateLaplaceWebGPU(dimension: number, scale: number): Float64Array {
    return this.generateLaplaceSIMD(dimension, scale);
  }
  ...
  getStats(): AccelerationStats {
    return {
      ...
      available: this.deviceInterface !== null || this.accelerator === 'simd' || this.accelerator === 'cpu',
      fallbackReason: this.accelerator === 'cpu' ? 'No hardware accelerators detected' : undefined,
    };
  }
  ...
  destroy() {
    ...
    if (this.deviceInterface && typeof this.deviceInterface.destroy === 'function') {
