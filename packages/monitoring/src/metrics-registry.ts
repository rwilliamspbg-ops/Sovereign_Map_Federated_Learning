...
export class MetricsRegistry {
  ...
  exportPrometheus(): string {
    ...
    // GPU metrics
    output += this.exportGaugeMetric('gpu_utilization_percent', this.gpu.gpuUtilization.get(), { device: 'primary' });
    output += this.exportGaugeMetric('gpu_memory_used_bytes', this.gpu.gpuMemoryUsed.get(), { device: 'primary' });
    output += this.exportGaugeMetric('gpu_memory_total_bytes', this.gpu.gpuMemoryTotal.get(), { device: 'primary' });
    output += this.exportHistogramMetric('gpu_noise_latency_seconds', this.gpu.noiseGenerationLatency);
    output += this.exportGaugeMetric('gpu_active_devices', this.gpu.activeGPUDevices.get());
    
    // Privacy metrics
    output += this.exportGaugeMetric('privacy_epsilon_remaining', this.privacy.epsilonRemaining.get());
    output += this.exportGaugeMetric('privacy_overhead_percent', this.privacy.privacyOverheadPercent.get());
    output += this.exportConstMetric('privacy_noise_injected_total', this.privacy.noiseInjectedCount.get());
    output += this.exportHistogramMetric('privacy_noise_magnitude', this.privacy.noiseMagnitude);
    
    // Consensus metrics
    output += this.exportGaugeMetric('consensus_participation_rate', this.consensus.participationRate.get());
    output += this.exportGaugeMetric('consensus_nodes_online', this.consensus.nodesOnline.get());
    output += this.exportHistogramMetric('consensus_round_duration_seconds', this.consensus.roundDuration);
    output += this.exportConstMetric('consensus_rounds_completed', this.consensus.roundsCompleted.get());
    
    // Byzantine metrics
    output += this.exportConstMetric('byzantine_nodes_detected_total', this.consensus.byzantineNodesDetected.get());
    output += this.exportGaugeMetric('byzantine_nodes_currently_detected', this.consensus.byzantineNodesList.get());
    
    // Network metrics
    output += this.exportConstMetric('network_partitions_detected_total', this.partition.partitionsDetected.get());
    output += this.exportHistogramMetric('network_partition_recovery_seconds', this.partition.recoveryTime);
    output += this.exportConstMetric('network_partition_recovery_success', this.partition.partitionRecoverySuccess.get());
    
    // System metrics
    output += this.exportGaugeMetric('system_memory_bytes', this.system.processMpemory.get());
    output += this.exportGaugeMetric('system_cpu_usage_percent', this.system.cpuUsage.get());
    output += this.exportHistogramMetric('system_network_latency_seconds', this.system.networkLatency);
    
    return output;
  }
  
  private exportGaugeMetric(name: string, value: number, labels?: Record<string, string>): string {
  ...
  private exportHistogramMetric(name: string, metric: HistogramMetric): string {
    ...
    output += `${name}_bucket{le="0.5"} ${stats.count * (stats.p50 <= 0.5 ? 1 : 0)}\n`;
    output += `${name}_bucket{le="1"} ${stats.count * (stats.p50 <= 1 ? 1 : 0)}\n`;
    output += `${name}_bucket{le="5"} ${stats.count * (stats.p50 <= 5 ? 1 : 0)}\n`;
  ...
  exportSLAStatus(): Record<string, boolean> {
    return {
      ...
      'consensus_participation_sla': this.consensus.participationRate.get() > 66.67,
      'network_latency_sla': this.system.networkLatency.getPercentile(99) < 100,
      'byzantine_tolerance_sla': this.consensus.byzantineNodesDetected.get() < 334, // <33%
    };
  }
}
