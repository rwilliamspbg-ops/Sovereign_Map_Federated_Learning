...
def start_training():
    ...
    with training_state["lock"]:
        if training_state["status"] == "training":
            return (
                jsonify({"status": "error", "message": "Training already in progress"}),
                400,
            )
...
def _run_training():
    ...
    try:
        ...
        for round_num in range(training_state["total_rounds"]):
            ...
            logger.info(
                f"Round {round_num + 1}/{training_state['total_rounds']}: "
        ...
        return jsonify(
            {
                ...
                "progress_percent": (
                    ...
                    / max(training_state["total_rounds"], 1)
                )
                * 100,
...
def cancel_training():
    ...
    with training_state["lock"]:
        if training_state["status"] != "training":
            return (
                jsonify({"status": "error", "message": "No training in progress"}),
                400,
            )
    ...
    return jsonify(
        {"status": "cancelled", "rounds_completed": training_state["current_round"]}
    )
