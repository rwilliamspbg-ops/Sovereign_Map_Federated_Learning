...
describe('IslandModeManager', () => {
  ...
  it('throws when queueing update while island mode is disabled', async () => {
    ...
    await manager.initialize();
    await expect(manager.queueUpdate({ location: 'x' }, 'proof-x')).rejects.toThrow('Island Mode not enabled');
  ...
  it('detects tampering via previous hash mismatch', async () => {
    ...
    await expect(manager.sync()).rejects.toThrow('Chain integrity violated at position 0');
  ...
  it('detects tampering via chained hash mismatch', async () => {
    ...
    await (manager as any).db.put(`update:${queued.id}`, JSON.stringify(parsed));
