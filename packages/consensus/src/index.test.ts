...
describe('ConsensusParticipant', () => {
  it('initializes and can accept a proposal when quorum is met', async () => {
    ...
    expect(network.messages[0].proposal.proof).toContain('zkp-1700000000000-4fzzzxjyl-');
  });

  it('broadcasts leave messages and handles proposal validation branches', async () => {
    const network = new MockNetwork();
    const participant = new ConsensusParticipant({ nodeId: 'n2', network, logger });
    ...
    await network.broadcast({ type: 'CONSENSUS_VOTE', proposalId: 'proposal-valid', voter: 'peer-1' });
    await participant.leave();

    expect(network.messages).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'CONSENSUS_VOTE', proposalId: 'proposal-valid', voter: 'n2' }),
  ...
  it('collects external votes and times out when quorum is not reached', async () => {
    ...
    const network = new MockNetwork();
    const participant = new ConsensusParticipant({ nodeId: 'n3', network, logger });

    await participant.initialize();
    (participant as any).config.quorumSize = 2;

    const quorumPromise = (participant as any).collectVotes('proposal-quorum', 1000);
    await network.broadcast({
      type: 'CONSENSUS_VOTE',
    ...
    const timeoutPromise = (participant as any).collectVotes('proposal-timeout', 1000);
    await network.broadcast({
      type: 'CONSENSUS_VOTE',
  ...
  it('returns a rejected consensus result when quorum is not met before timeout', async () => {
    ...
    const network = new MockNetwork();
    const participant = new ConsensusParticipant({ nodeId: 'n4', network, logger });
    ...
    const result = await pending;
    expect(result).toEqual({ accepted: false, roundId: 'round-timeout', reward: 0, votes: 1 });
