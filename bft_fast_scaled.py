...
class FastAggregation:
    ...
    def krum_fast(updates, byzantine_count=None):
        ...
        good_idx = sorted(range(len(distances)), key=lambda i: distances[i])[:-byzantine_count]
        if len(good_idx) == 0:
            return np.mean(updates, axis=0)
...
class FastScaledBFT:
    ...
    def run_config(self, bft_pct, attack, agg):
        ...
        self.byzantine_nodes = set(np.random.choice(self.NUM_NODES, num_byz, replace=False))
        ...
        threshold = 85.0 if bft_pct <= 10 else (80.0 if bft_pct <= 20 else (75.0 if bft_pct <= 30 else 70.0))
    ...
    def run_all(self):
        ...
        for bft in bft_levels:
            for attack in attacks:
                for agg in aggs:
                    config_num += 1
                    print(f"  [{config_num:2d}/{total}] {bft:2d}% | {attack:12s} | {agg:6s} | ", end="", flush=True)
