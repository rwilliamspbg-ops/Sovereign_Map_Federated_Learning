    ...
    def can_communicate(self, node1, node2, round_num=0):
        ...
        if self.ptype == "binary":
            ...
            if len(self.partition_a) < new_size:
                ...
                candidates = self.partition_b - set(
                    random.sample(
                        list(self.partition_b),
    ...
    def get_partition_aggregates(self, round_num):
        ...
        if self.ptype == "binary":
            ...
            if self.partition_sim.isolated:
                return [
                    self.partition_sim.partition_a,
                    self.partition_sim.partition_b,
        ...
        for ptype in ptypes:
            ...
            for bft in bft_levels:
                test = NetworkPartitionTest(
                    num_nodes=scale, partition_type=ptype, bft_pct=bft, rounds=30
                )
                ...
                detected = (
                    "Detected"
                    if metrics["max"] - metrics["min"] > 10
                    else "Not detected"
                )

                print(
                    f"      Byzantine {bft:2d}%: Final {metrics['final']:5.1f}% | "
