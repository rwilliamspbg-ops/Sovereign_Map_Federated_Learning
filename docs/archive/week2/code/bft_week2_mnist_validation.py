...
class MNISTDataHandler:
        ...
        if self.iid:
            ...
            for node in range(self.num_nodes):
                ...
                classes = np.random.choice(
                    10, size=np.random.randint(2, 4), replace=False
                )
                node_idx = []
                for cls in classes:
    ...
    def get_test_accuracy(self, global_model, num_samples=1000):
        ...
        similarity = np.mean(
            np.abs(np.corrcoef(global_model, np.mean(X_test, axis=0))[0, 1:])
        )
...
class MNISTFederatedTest:
    ...
    def run_config(self, bft_pct, attack_type, agg_method):
        ...
        avg_last_3 = (
            np.mean(self.accuracies[-3:]) if len(self.accuracies) >= 3 else final
        )
        ...
        for bft in bft_levels:
            for atk in attacks:
                for agg in aggs:
                    ...
                    self.results.append(
                        {"bft_pct": bft, "attack": atk, "agg": agg, **res}
                    )
...
if __name__ == "__main__":
    ...
    for num_nodes in scales:
        for is_iid in data_types:
            ...
            print(
                f"  Convergence: {metrics['conv_rate']:3d}% | Accuracy: {metrics['avg_acc']:5.1f}% | Time: {elapsed:6.1f}s\n"
            )
    ...
    for key in sorted(all_results.keys()):
        ...
        print(
            f"  {scale:<12} {dtype:<10} {m['conv_rate']:>6}%{'':<8} {m['avg_acc']:>7.1f}%"
        )
