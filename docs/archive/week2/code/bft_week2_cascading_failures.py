...
class CascadingFailureTest:
    def __init__(self, num_nodes=200, cascade_type="avalanche", initial_fail_pct=5, bft_pct=0, rounds=40):
        self.N = num_nodes
        self.ctype = cascade_type
    ...
    def apply_cascading_failures(self, r):
        ...
        if self.ctype == "avalanche":
            ...
            nodes_to_recover = [n for n, rec_r in self.recovery_schedule.items() if rec_r == r]
            ...
            if num_byz > 0 and num_failed < self.N * 0.3:
                ...
                if random.random() < new_fail_prob:
                    candidates = list(set(range(self.N)) - self.failed_nodes - set(range(num_byz)))
                    if candidates:
                        node = random.choice(candidates)
                        self.failed_nodes.add(node)
    ...
    def run_all(self):
        ...
        return {
            ...
            'avg_last_5': np.mean(self.accuracies[-5:]) if len(self.accuracies) >= 5 else self.accuracies[-1],
...
if __name__ == "__main__":
        ...
        for ctype in cascade_types:
            ...
            for init_fail in initial_fails:
                for bft in bft_levels:
                    test = CascadingFailureTest(num_nodes=scale, cascade_type=ctype, 
                                               initial_fail_pct=init_fail, bft_pct=bft, rounds=40)
                    ...
                    print(f"      {status} Init {init_fail}% + Byz {bft}%: "
    ...
    print(f"  Cascade Containment: {'[OK] Contained <30%' if avg_max_failed < 30 else '⚠ Exceeds 30%'}")
