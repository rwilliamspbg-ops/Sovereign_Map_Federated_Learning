...
class RecoveryMetrics:
    ...
    def compute_recovery_metrics(self):
        ...
        metrics["recovery_time_rounds"] = (
            self.recovery_time if self.recovery_time else -1
        )
        metrics["amplification_factor"] = (
            np.mean(self.amplification_factor) if self.amplification_factor else 0
        )
...
def run_boundary_sweep():
    ...
    print(
        f"  {'Byzantine %':<15} {'Final Acc':<15} {'Avg Acc':<15} {'Min Acc':<15} {'Status':<12}"
    )
    print(f"  {'-'*70}")
    ...
    for bft_pct in byzantine_levels:
        ...
        print(
            f"  {bft_pct}%{'':<12} {final:5.1f}%{'':<10} {avg:5.1f}%{'':<10} {min_acc:5.1f}%{'':<10} {status:<12}"
        )
