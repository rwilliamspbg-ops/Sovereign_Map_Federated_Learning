...
class UltraScaleTest:
    ...
    def run_round(self, r):
        ...
        if self.strategy == "full":
            ...
            agg_result = ScalingStrategy.hierarchical_aggregation(
                updates, group_size=50
            )
...
if __name__ == "__main__":
    ...
    for scale in scales:
        ...
        for strategy in strategies:
            ...
            for bft in bft_levels:
                test = UltraScaleTest(
                    num_nodes=scale, agg_strategy=strategy, bft_pct=bft, rounds=10
                )
    ...
    for scale in scales:
        ...
        hier_t = np.mean(
            [results[f"{scale}_hierarchical_{b}"]["time"] for b in bft_levels]
        )
    ...
    for scale in scales:
        ...
        samp_speedup = full_t / np.mean(
            [results[f"{scale}_sampled_{b}"]["time"] for b in bft_levels]
        )
        hier_speedup = full_t / np.mean(
            [results[f"{scale}_hierarchical_{b}"]["time"] for b in bft_levels]
        )
    ...
    for scale in scales:
        ...
        hier_acc = np.mean(
            [results[f"{scale}_hierarchical_{b}"]["avg"] for b in bft_levels]
        )
