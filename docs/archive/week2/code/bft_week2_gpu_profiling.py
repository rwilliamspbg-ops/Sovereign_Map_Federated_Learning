...
class GPUProfilingTest:
        ...
        for op in operations:
            for ds in data_sizes:
                for bs in batch_sizes:
                    if op == "aggregation":
                        cpu_t, gpu_t = profiler.profile_aggregation(ds, bs, iterations=100)
                    elif op == "gradients":
                        cpu_t, gpu_t = profiler.profile_gradients(ds, bs, iterations=100)
                    else:
                        # Skip very large distance calc for speed
                        if ds > 1000 or bs > 200:
                            continue
                        cpu_t, gpu_t = profiler.profile_distance_calc(ds, bs, iterations=10)
    ...
    def print_results(self):
        ...
        for op in ["aggregation", "gradients", "distance"]:
            ...
            print(f"  " + "-"*116)
            print(f"  {'Data Size':<12} {'Batch Size':<12} {'CPU Time (ms)':<16} {'GPU Time (ms)':<16} {'Speedup':<12}")
            print(f"  {'-'*70}")
            ...
            for res in op_results:
                ...
                print(f"  {res['data_size']:<12} {res['batch_size']:<12} {cpu_ms:<16.3f} "
