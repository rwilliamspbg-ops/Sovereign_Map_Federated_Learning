...
class TPMSimulator:
    ...
    def extend_measurement(self, measurement: bytes, pcr_index: int = TPMConfig.PCR_APPLICATION):
...
class AttestationService:
    ...
    def verify_node_attestation(self, node_id: int, quote: Dict, nonce: str) -> Tuple[bool, str]:
    ...
    def get_byzantine_node_confidence(self, node_id: int, Byzantine_suspect: bool) -> float:
...
def attestation_aware_aggregation(
    ...
    for update in verified_updates:
        ...
        node_id = update['node_id']
        confidence = attestation_service.get_byzantine_node_confidence(
            node_id, False
        )
