...
class Aggregation:
    ...
    def krum_fast(w, byz_cnt=None):
        ...
        for i in range(len(w)):
            d = sum(np.linalg.norm(w[i] - w[j]) for j in range(min(10, len(w))) if i != j)
            dist.append(d)
    ...
    def run_all(self):
        ...
        for bft in bft_lvls:
            for atk in attacks:
                for agg in aggs:
                    config_num += 1
                    print(f"  [{config_num:2d}/{total}] {bft:2d}% | {atk:12s} | {agg:10s} | ", end="", flush=True)
                    
                    res = self.run_config(bft, atk, agg)
                    self.results.append(res)
                    
                    status = "OK" if res['converged'] else "XX"
                    print(f"[{status}] Acc: {res['final']:6.2f}% Avg5: {res['avg_last_5']:6.2f}%")
