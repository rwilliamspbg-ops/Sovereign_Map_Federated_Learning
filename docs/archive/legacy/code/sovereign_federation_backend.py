...
class SovereignFederationBackend:
    ...
    def __init__(self, port: int = 8000):
        ...
        self.app.config["SECRET_KEY"] = os.getenv(
            "SECRET_KEY", "sovereign-federation-key"
        )
        CORS(self.app)
    ...
    def _setup_routes(self):
        ...
        def update_metrics():
            ...
            return jsonify(
                {"status": "success", "nodes_updated": len(data.get("nodes", []))}
            )
        ...
        def analyze_threat():
            ...
            self.socketio.emit(
                "threat_analysis", self.last_threat_analysis, broadcast=True
            )
    ...
    def run(self):
        ...
        print(
            f"      - Threat Analysis: http://localhost:{self.port}/threat/analyze (POST)"
        )
