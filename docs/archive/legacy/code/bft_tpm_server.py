            ...
            test_results["results"] = [
                {
                    k: v
                    for k, v in r.items()
                    if k not in ["accuracy_curve", "loss_curve"]
                }
                for r in results
            ]
    ...
    return jsonify(
        {
            ...
            "verification_success_rate": (
                current_test.metrics["verified_quotes"]
                / current_test.metrics["total_quotes"]
                if current_test.metrics["total_quotes"] > 0
                else 0
            ),
            "nodes_total": current_test.NUM_NODES,
            "configurations": len(current_test.BYZANTINE_PERCENTAGES)
            * len(current_test.AGGREGATION_METHODS),
        }
    )
