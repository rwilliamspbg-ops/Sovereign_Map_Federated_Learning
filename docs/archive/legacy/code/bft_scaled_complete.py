...
class ScaledBFTTest:
    ...
    def run_round(self, round_num, bft_pct, attack_type, agg_method):
        ...
        if len(updates) > 0:
            ...
            aggregated = AggregationMethods.apply(
                np.array(updates), method=agg_method, byzantine_count=byzantine_count
            )
    ...
    def run_all(self):
        ...
        print(
            f"  Byzantine Levels: {len(self.BFT_LEVELS)} | Attacks: {len(self.ATTACKS)} | Methods: {len(self.AGG_METHODS)}"
        )
        ...
        for bft in self.BFT_LEVELS:
            for attack in self.ATTACKS:
                for agg in self.AGG_METHODS:
                    config_num += 1
                    print(
                        f"  [{config_num:3d}/{total_configs}] {bft:2d}% | {attack:12s} | {agg:10s} | ",
                        end="",
                        flush=True,
                    )
