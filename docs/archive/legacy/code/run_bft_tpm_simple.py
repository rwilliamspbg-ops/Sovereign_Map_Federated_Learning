...
class SimpleBFTTest:
    ...
    def print_config_header(
        self, config_num: int, byzantine_pct: float, agg_method: str
    ):
        print("\n" + "-" * 90)
        print(
            f"  Config {config_num}/12 | {byzantine_pct}% Byzantine | {agg_method.upper()}"
        )
        print("-" * 90 + "\n")
    ...
    def print_metrics(self, round_num: int, results: List[Dict], byzantine_pct: float):
        ...
        print(
            f"    +- Verified: {int(avg_verified)}/{self.NUM_NODES} ({avg_attestation_rate:.1%})"
        )
        print(
            f"    +- Byzantine: {byzantine_pct}% | Quotes: {self.metrics['total_quotes']:,}"
        )
        rate = (
            (self.metrics["verified_quotes"] / self.metrics["total_quotes"]) * 100
        ...
        for byzantine_pct in self.BYZANTINE_PERCENTAGES:
            for agg_method in self.AGGREGATION_METHODS:
                ...
                for round_num in range(1, self.ROUNDS + 1):
                    ...
                    if (
                        round_num % self.UPDATE_INTERVAL == 0
                        or round_num == self.ROUNDS
                    ):
                ...
                print(
                    f"  Config {config_num} Complete: {status} | Final Acc: {final_accuracy:.2f}%\n"
                )
