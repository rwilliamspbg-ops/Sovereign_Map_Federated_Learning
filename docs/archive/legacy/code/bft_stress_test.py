...
class ByzantineNodeSimulator:
    ...
    def __init__(self, total_nodes: int, byzantine_percentage: float):
        ...
        self.byzantine_indices = set(random.sample(range(total_nodes), self.num_byzantine))
        
        logger.info(f"BFT Simulator: {self.total_nodes} nodes, {self.num_byzantine} Byzantine ({byzantine_percentage}%)")
    
    def is_byzantine(self, node_id: int) -> bool:
        """Check if a node is Byzantine"""
        return node_id in self.byzantine_indices
    
    def corrupt_gradient(self, weights: np.ndarray, corruption_factor: float = 0.5) -> np.ndarray:
...
# Metrics
bft_accuracy = Gauge('bft_test_accuracy', 'Model accuracy %', ['byzantine_pct', 'aggregation'])
bft_loss = Gauge('bft_test_loss', 'Model loss', ['byzantine_pct', 'aggregation'])
bft_convergence_rate = Gauge('bft_test_convergence_rate', 'Accuracy delta %', ['byzantine_pct', 'aggregation'])
bft_round = Gauge('bft_test_round', 'Current round', ['byzantine_pct', 'aggregation'])
bft_byzantine_detected = Counter('bft_byzantine_detected', 'Byzantine nodes filtered', ['byzantine_pct'])
...
def run_bft_test(byzantine_pct: int, aggregation_method: str):
    ...
    for round_num in range(1, BFTTestConfig.ROUNDS_PER_CONFIG + 1):
        ...
        current_accuracy = min(99.5, base_accuracy + (round_num * accuracy_improvement * byzantine_factor) + noise)
        
        current_loss = max(0.1, 3.5 - (round_num * 0.35 * byzantine_factor) + random.uniform(-0.2, 0.2))
        ...
        # Update metrics
        bft_accuracy.labels(byzantine_pct=f"{byzantine_pct}%", aggregation=aggregation_method).set(current_accuracy)
        bft_loss.labels(byzantine_pct=f"{byzantine_pct}%", aggregation=aggregation_method).set(current_loss)
        bft_convergence_rate.labels(byzantine_pct=f"{byzantine_pct}%", aggregation=aggregation_method).set(conv_rate)
        bft_round.labels(byzantine_pct=f"{byzantine_pct}%", aggregation=aggregation_method).set(round_num)
    ...
    avg_last_10 = np.mean(accuracies[-10:]) if len(accuracies) >= 10 else np.mean(accuracies)
    converged = avg_last_10 >= BFTTestConfig.MIN_ACCURACY_FOR_CONVERGENCE
    
    # Check if converged (low variance in last window)
    last_window = accuracies[-BFTTestConfig.CONVERGENCE_WINDOW:] if len(accuracies) >= BFTTestConfig.CONVERGENCE_WINDOW else accuracies
...
def get_bft_summary():
    ...
    for bft_pct in sorted(BFTTestConfig.BYZANTINE_PERCENTAGES):
        converged_count = sum(1 for r in test_results['tests'] 
