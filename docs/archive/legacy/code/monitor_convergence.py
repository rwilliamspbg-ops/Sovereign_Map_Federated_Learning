...
class ConvergenceMonitor:
        ...
        try:
            result = subprocess.run(
                [
                    "docker",
                    "logs",
                    "sovereign_map_federated_learning-backend-1",
                    "--tail",
                    str(n),
                ],
                capture_output=True,
    ...
    def display_header(self):
        ...
        print(
            "║" + " " * 15 + "Federated Learning with 50 Node Agents" + " " * 25 + "║"
        )
        print("╚" + "═" * 78 + "╝")
    ...
    def display_convergence(self, data):
        ...
        if rounds:
            ...
            print(
                f"{'Round':<8} {'Timestamp':<20} {'Accuracy':<12} {'Loss':<10} {'Trend':<10}"
            )
            print("-" * 60)
            ...
            for i in range(max(0, len(rounds) - 10), len(rounds)):
                ...
                print(
                    f"{round_num:<8} {str(datetime.now()):<20} {acc:>10.2f}% {loss:>8.4f}  {trend:<10}"
                )
        else:
    ...
    def display_metrics(self, metrics):
        ...
        if acc_hist:
            print(
                f"Recent Accuracies: {' -> '.join([f'{a:.1f}%' for a in acc_hist[-5:]])}"
            )
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "-a",
                    "--filter",
                    "label=com.docker.compose.project=sovereign_map_federated_learning",
                    "--format",
                    "{{.Names}}\t{{.Status}}",
                ],
                capture_output=True,
