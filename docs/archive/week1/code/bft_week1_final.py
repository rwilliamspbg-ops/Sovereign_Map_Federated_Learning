...
class Week1BFTDemo:
    ...
    def run_all(self):
        ...
        for bft in self.BFT_LEVELS:
            for attack in self.ATTACKS:
                config_num += 1
                print(
                    f"  [{config_num:2d}/24] {bft:2d}% BFT | {attack:15s} | ",
                    end="",
                    flush=True,
                )
    ...
    def summary(self):
        ...
        if self.tpm.quotes_created > 0:
            print(
                f"  Verification Rate: {self.tpm.quotes_verified/self.tpm.quotes_created:.1%}\n"
            )
        ...
        print(
            f"  Real Byzantine attacks (4 types: sign-flip, label-flip, free-ride, amplification)"
        )
