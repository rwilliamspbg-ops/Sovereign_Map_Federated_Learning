...
class NetworkSimulator:
    ...
    def stats(self) -> Dict:
        ...
        delivery_rate = 1.0 - (self.metrics['total_packet_loss'] + self.metrics['total_timeouts']) / total if total > 0 else 1.0
        return {
            'total': total,
...
class Week1FastBFTTest:
    ...
    def run_round(self, round_num: int, byzantine_pct: float, attack_type: str) -> Dict:
        ...
        current_accuracy = min(
            99.5,
            base_accuracy + improvement - attack_impact - network_impact + attestation_boost + random.uniform(-0.5, 0.5)
        )
        
        current_loss = max(
            0.1,
            3.5 - (round_num * 0.35) + (actual_byzantine_pct * 2.0) + random.uniform(-0.2, 0.2)
        )
    ...
    def print_summary(self):
        ...
        for bft in sorted(self.BYZANTINE_PERCENTAGES):
            ...
            if all(not c['converged'] for c in configs):
                print(f"  [THRESHOLD] Exceeded at {bft}% (All {len(configs)} attacks failed)")
                break
