...
def main():
    ...
    print(f"   Total Configurations: {len(test.BYZANTINE_PERCENTAGES) * len(test.AGGREGATION_METHODS)}")
    print(f"   Total FL Rounds: {test.ROUNDS * len(test.BYZANTINE_PERCENTAGES) * len(test.AGGREGATION_METHODS)}")
    print()
    ...
    try:
        ...
        print("Critical Byzantine Threshold: {}%".format(
            summary['critical_threshold'] if summary['critical_threshold'] else "Not reached"
        ))
        print()
        
        print("🔐 ATTESTATION VERIFICATION RESULTS:")
        print(f"   Total Quotes Generated: {summary['attestation_metrics']['total_quotes']:,}")
        print(f"   Successfully Verified: {summary['attestation_metrics']['verified_quotes']:,}")
        print(f"   Failed Verifications: {summary['attestation_metrics']['failed_verifications']}")
        print(f"   Verification Success Rate: {summary['attestation_metrics']['verification_success_rate']:.2%}")
        print()
        ...
        for bft_pct in test.BYZANTINE_PERCENTAGES:
            ...
            print(f"   {bft_pct:2d}% Byzantine {converged_str} | "
